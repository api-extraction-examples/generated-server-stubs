...
@Validated
@Tag(name = "Project statuses", description = "*Deprecated: new integrations should prefer using [status updates](/docs/asana-statuses)*  A project status is an update on the progress of a particular project, and is sent out to all project followers when created. These updates include both text describing the update and a color code intended to represent the overall state of the project: \"green\" for projects that are on track, \"yellow\" for projects at risk, \"red\" for projects that are behind, and \"blue\" for projects on hold.  Project statuses can be created and deleted, but not modified.")
public interface ProjectStatusesApi {
    ...
    @RequestMapping(
        ...
        value = "/project_statuses/{project_status_gid}",
        produces = { "application/json" }
    )
    
    default ResponseEntity<DeleteAttachment200Response> deleteProjectStatus(
        @Parameter(name = "project_status_gid", description = "The project status update to get.", required = true, in = ParameterIn.PATH) @PathVariable("project_status_gid") String projectStatusGid,
