/*
 * AWS Identity and Access Management
 *
 * <fullname>Identity and Access Management</fullname> <p>Identity and Access Management (IAM) is a web service for securely controlling access to Amazon Web Services services. With IAM, you can centrally manage users, security credentials such as access keys, and permissions that control which Amazon Web Services resources users and applications can access. For more information about IAM, see <a href=\"http://aws.amazon.com/iam/\">Identity and Access Management (IAM)</a> and the <a href=\"https://docs.aws.amazon.com/IAM/latest/UserGuide/\">Identity and Access Management User Guide</a>.</p>
 *
 * API version: 2010-05-08
 * Contact: mike.ralphson@gmail.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi


import (
	"time"
)



// RoleDetail - <p>Contains information about an IAM role, including all of the role's policies.</p> <p>This data type is used as a response element in the <a>GetAccountAuthorizationDetails</a> operation.</p>
type RoleDetail struct {

	Path string `json:"Path,omitempty"`

	RoleName string `json:"RoleName,omitempty"`

	RoleId string `json:"RoleId,omitempty"`

	// <p>The Amazon Resource Name (ARN). ARNs are unique identifiers for Amazon Web Services resources.</p> <p>For more information about ARNs, go to <a href=\"https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html\">Amazon Resource Names (ARNs)</a> in the <i>Amazon Web Services General Reference</i>. </p>
	Arn string `json:"Arn,omitempty"`

	CreateDate time.Time `json:"CreateDate,omitempty"`

	AssumeRolePolicyDocument string `json:"AssumeRolePolicyDocument,omitempty"`

	InstanceProfileList Array `json:"InstanceProfileList,omitempty"`

	RolePolicyList Array `json:"RolePolicyList,omitempty"`

	AttachedManagedPolicies Array `json:"AttachedManagedPolicies,omitempty"`

	PermissionsBoundary RolePermissionsBoundary `json:"PermissionsBoundary,omitempty"`

	Tags Array `json:"Tags,omitempty"`

	RoleLastUsed RoleDetailRoleLastUsed `json:"RoleLastUsed,omitempty"`
}

// AssertRoleDetailRequired checks if the required fields are not zero-ed
func AssertRoleDetailRequired(obj RoleDetail) error {
	if err := AssertstringRequired(obj.Path); err != nil {
		return err
	}
	if err := AssertstringRequired(obj.RoleName); err != nil {
		return err
	}
	if err := AssertstringRequired(obj.RoleId); err != nil {
		return err
	}
	if err := Asserttime.TimeRequired(obj.CreateDate); err != nil {
		return err
	}
	if err := AssertstringRequired(obj.AssumeRolePolicyDocument); err != nil {
		return err
	}
	if err := AssertarrayRequired(obj.InstanceProfileList); err != nil {
		return err
	}
	if err := AssertarrayRequired(obj.RolePolicyList); err != nil {
		return err
	}
	if err := AssertarrayRequired(obj.AttachedManagedPolicies); err != nil {
		return err
	}
	if err := AssertRolePermissionsBoundaryRequired(obj.PermissionsBoundary); err != nil {
		return err
	}
	if err := AssertarrayRequired(obj.Tags); err != nil {
		return err
	}
	if err := AssertRoleDetailRoleLastUsedRequired(obj.RoleLastUsed); err != nil {
		return err
	}
	return nil
}

// AssertRoleDetailConstraints checks if the values respects the defined constraints
func AssertRoleDetailConstraints(obj RoleDetail) error {
	return nil
}
