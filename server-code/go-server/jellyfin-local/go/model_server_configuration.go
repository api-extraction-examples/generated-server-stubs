...
// AssertServerConfigurationRequired checks if the required fields are not zero-ed
func AssertServerConfigurationRequired(obj ServerConfiguration) error {
	if obj.ContentTypes != nil {
		for _, el := range *obj.ContentTypes {
			if err := AssertNameValuePairRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.MetadataOptions != nil {
		for _, el := range *obj.MetadataOptions {
			if err := AssertMetadataOptionsRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.PathSubstitutions != nil {
		for _, el := range *obj.PathSubstitutions {
			if err := AssertPathSubstitutionRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.PluginRepositories != nil {
		for _, el := range *obj.PluginRepositories {
			if err := AssertRepositoryInfoRequired(el); err != nil {
				return err
			}
		}
	}
	if err := AssertVersionRequired(obj.PreviousVersion); err != nil {
		return err
	}
	return nil
}

// AssertServerConfigurationConstraints checks if the values respects the defined constraints
func AssertServerConfigurationConstraints(obj ServerConfiguration) error {
	return nil
}
