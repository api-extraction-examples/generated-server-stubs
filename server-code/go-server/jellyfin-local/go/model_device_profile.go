...
// AssertDeviceProfileRequired checks if the required fields are not zero-ed
func AssertDeviceProfileRequired(obj DeviceProfile) error {
	if obj.CodecProfiles != nil {
		for _, el := range *obj.CodecProfiles {
			if err := AssertCodecProfileRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.ContainerProfiles != nil {
		for _, el := range *obj.ContainerProfiles {
			if err := AssertContainerProfileRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.DirectPlayProfiles != nil {
		for _, el := range *obj.DirectPlayProfiles {
			if err := AssertDirectPlayProfileRequired(el); err != nil {
				return err
			}
		}
	}
	if err := AssertDeviceIdentificationRequired(obj.Identification); err != nil {
		return err
	}
	if obj.ResponseProfiles != nil {
		for _, el := range *obj.ResponseProfiles {
			if err := AssertResponseProfileRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.SubtitleProfiles != nil {
		for _, el := range *obj.SubtitleProfiles {
			if err := AssertSubtitleProfileRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.TranscodingProfiles != nil {
		for _, el := range *obj.TranscodingProfiles {
			if err := AssertTranscodingProfileRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.XmlRootAttributes != nil {
		for _, el := range *obj.XmlRootAttributes {
			if err := AssertXmlAttributeRequired(el); err != nil {
				return err
			}
		}
	}
...
// AssertDeviceProfileConstraints checks if the values respects the defined constraints
func AssertDeviceProfileConstraints(obj DeviceProfile) error {
	return nil
}
