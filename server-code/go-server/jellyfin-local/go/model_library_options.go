...
// AssertLibraryOptionsRequired checks if the required fields are not zero-ed
func AssertLibraryOptionsRequired(obj LibraryOptions) error {
	if obj.PathInfos != nil {
		for _, el := range *obj.PathInfos {
			if err := AssertMediaPathInfoRequired(el); err != nil {
				return err
			}
		}
	}
	if obj.TypeOptions != nil {
		for _, el := range *obj.TypeOptions {
			if err := AssertTypeOptionsRequired(el); err != nil {
				return err
			}
		}
	}
...
// AssertLibraryOptionsConstraints checks if the values respects the defined constraints
func AssertLibraryOptionsConstraints(obj LibraryOptions) error {
	return nil
}
