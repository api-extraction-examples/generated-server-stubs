...
// VideoHlsAPIController binds http requests to an api service and writes the service results to the http response
type VideoHlsAPIController struct {
	service VideoHlsAPIServicer
	errorHandler ErrorHandler
}

// VideoHlsAPIOption for how the controller is set up.
type VideoHlsAPIOption func(*VideoHlsAPIController)

// WithVideoHlsAPIErrorHandler inject ErrorHandler into controller
func WithVideoHlsAPIErrorHandler(h ErrorHandler) VideoHlsAPIOption {
	return func(c *VideoHlsAPIController) {
		c.errorHandler = h
	}
}

// NewVideoHlsAPIController creates a default api controller
func NewVideoHlsAPIController(s VideoHlsAPIServicer, opts ...VideoHlsAPIOption) Router {
	controller := &VideoHlsAPIController{
		service:      s,
...
// Routes returns all the api routes for the VideoHlsAPIController
func (c *VideoHlsAPIController) Routes() Routes {
	return Routes{
		"GetLiveHlsStream": Route{
			strings.ToUpper("Get"),
...
func (c *VideoHlsAPIController) GetLiveHlsStream(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	query := r.URL.Query()
	...
	var enableSubtitlesInManifestParam *bool
	if query.Has("enableSubtitlesInManifest") {
		param, err := parseBoolParameter(
			query.Get("enableSubtitlesInManifest"),
			WithParse[bool](parseBool),
		)
		if err != nil {
			c.errorHandler(w, r, &ParsingError{Err: err}, nil)
