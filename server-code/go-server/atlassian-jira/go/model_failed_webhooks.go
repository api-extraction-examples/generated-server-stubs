/*
 * The Jira Cloud platform REST API
 *
 * Jira Cloud platform REST API documentation
 *
 * API version: 1001.0.0-SNAPSHOT
 * Contact: ecosystem@atlassian.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi




// FailedWebhooks - A page of failed webhooks.
type FailedWebhooks struct {

	// The maximum number of items on the page. If the list of values is shorter than this number, then there are no more pages.
	MaxResults int32 `json:"maxResults"`

	// The URL to the next page of results. Present only if the request returned at least one result.The next page may be empty at the time of receiving the response, but new failed webhooks may appear in time. You can save the URL to the next page and query for new results periodically (for example, every hour).
	Next string `json:"next,omitempty"`

	// The list of webhooks.
	Values []FailedWebhook `json:"values"`
}

// AssertFailedWebhooksRequired checks if the required fields are not zero-ed
func AssertFailedWebhooksRequired(obj FailedWebhooks) error {
	elements := map[string]interface{}{
		"maxResults": obj.MaxResults,
		"values": obj.Values,
	}
	for name, el := range elements {
		if isZero := IsZeroValue(el); isZero {
			return &RequiredError{Field: name}
		}
	}

	for _, el := range obj.Values {
		if err := AssertFailedWebhookRequired(el); err != nil {
			return err
		}
	}
	return nil
}

// AssertFailedWebhooksConstraints checks if the values respects the defined constraints
func AssertFailedWebhooksConstraints(obj FailedWebhooks) error {
	return nil
}
