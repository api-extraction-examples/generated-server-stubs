/*
 * The Jira Cloud platform REST API
 *
 * Jira Cloud platform REST API documentation
 *
 * API version: 1001.0.0-SNAPSHOT
 * Contact: ecosystem@atlassian.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi




type CustomFieldContextDefaultValue struct {

	// The ID of the default cascading option.
	CascadingOptionId string `json:"cascadingOptionId,omitempty"`

	// The ID of the context.
	ContextId string `json:"contextId"`

	// The ID of the default option.
	OptionId string `json:"optionId"`

	Type string `json:"type"`

	// The list of IDs of the default options.
	OptionIds []string `json:"optionIds"`

	// The ID of the default user.
	AccountId string `json:"accountId"`

	UserFilter UserFilter `json:"userFilter"`

	// The IDs of the default users.
	AccountIds []string `json:"accountIds"`

	// The ID of the the default group.
	GroupId string `json:"groupId"`

	// The IDs of the default groups.
	GroupIds []string `json:"groupIds"`

	// The default date in ISO format. Ignored if `useCurrent` is true.
	Date string `json:"date,omitempty"`

	// Whether to use the current date.
	UseCurrent bool `json:"useCurrent,omitempty"`

	// The default date-time in ISO format. Ignored if `useCurrent` is true.
	DateTime string `json:"dateTime,omitempty"`

	// The default URL.
	Url string `json:"url"`

	// The ID of the default project.
	ProjectId string `json:"projectId"`

	// The default floating-point number.
	Number float64 `json:"number"`

	// The default labels value.
	Labels []string `json:"labels"`

	// The default text. The maximum length is 254 characters.
	Text string `json:"text,omitempty"`

	// The ID of the default version.
	VersionId string `json:"versionId"`

	// The order the pickable versions are displayed in. If not provided, the released-first order is used. Available version orders are `\"releasedFirst\"` and `\"unreleasedFirst\"`.
	VersionOrder string `json:"versionOrder,omitempty"`

	// The IDs of the default versions.
	VersionIds []string `json:"versionIds"`

	// List of string values. The maximum length for a value is 254 characters.
	Values []string `json:"values,omitempty"`

	// The default JSON object.
	Object map[string]interface{} `json:"object,omitempty"`
}

// AssertCustomFieldContextDefaultValueRequired checks if the required fields are not zero-ed
func AssertCustomFieldContextDefaultValueRequired(obj CustomFieldContextDefaultValue) error {
	elements := map[string]interface{}{
		"contextId": obj.ContextId,
		"optionId": obj.OptionId,
		"type": obj.Type,
		"optionIds": obj.OptionIds,
		"accountId": obj.AccountId,
		"userFilter": obj.UserFilter,
		"accountIds": obj.AccountIds,
		"groupId": obj.GroupId,
		"groupIds": obj.GroupIds,
		"url": obj.Url,
		"projectId": obj.ProjectId,
		"number": obj.Number,
		"labels": obj.Labels,
		"versionId": obj.VersionId,
		"versionIds": obj.VersionIds,
	}
	for name, el := range elements {
		if isZero := IsZeroValue(el); isZero {
			return &RequiredError{Field: name}
		}
	}

	if err := AssertUserFilterRequired(obj.UserFilter); err != nil {
		return err
	}
	return nil
}

// AssertCustomFieldContextDefaultValueConstraints checks if the values respects the defined constraints
func AssertCustomFieldContextDefaultValueConstraints(obj CustomFieldContextDefaultValue) error {
	return nil
}
