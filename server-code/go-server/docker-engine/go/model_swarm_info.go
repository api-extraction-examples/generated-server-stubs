...
// AssertSwarmInfoRequired checks if the required fields are not zero-ed
func AssertSwarmInfoRequired(obj SwarmInfo) error {
	if obj.Cluster != nil {
		if err := AssertClusterInfoRequired(*obj.Cluster); err != nil {
			return err
		}
	}
	if obj.RemoteManagers != nil {
		for _, el := range *obj.RemoteManagers {
			if err := AssertPeerNodeRequired(el); err != nil {
				return err
			}
		}
	}
...
// AssertSwarmInfoConstraints checks if the values respects the defined constraints
func AssertSwarmInfoConstraints(obj SwarmInfo) error {
	return nil
}
