...
// AssertHostConfigRequired checks if the required fields are not zero-ed
func AssertHostConfigRequired(obj HostConfig) error {
	for _, el := range obj.BlkioDeviceReadBps {
		if err := AssertThrottleDeviceRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.BlkioDeviceReadIOps {
		if err := AssertThrottleDeviceRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.BlkioDeviceWriteBps {
		if err := AssertThrottleDeviceRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.BlkioDeviceWriteIOps {
		if err := AssertThrottleDeviceRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.BlkioWeightDevice {
		if err := AssertResourcesBlkioWeightDeviceInnerRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Devices {
		if err := AssertDeviceMappingRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Ulimits {
		if err := AssertResourcesUlimitsInnerRequired(el); err != nil {
			return err
		}
	}
	if err := AssertHostConfigAllOfLogConfigRequired(obj.LogConfig); err != nil {
		return err
	}
	...
	if err := AssertRestartPolicyRequired(obj.RestartPolicy); err != nil {
		return err
	}
	return nil
}

// AssertHostConfigConstraints checks if the values respects the defined constraints
func AssertHostConfigConstraints(obj HostConfig) error {
	if obj.BlkioWeight < 0 {
		return &ParsingError{Err: errors.New(errMsgMinValueConstraint)}
	}
	if obj.BlkioWeight > 1000 {
		return &ParsingError{Err: errors.New(errMsgMaxValueConstraint)}
	}
	if obj.MemorySwappiness < 0 {
		return &ParsingError{Err: errors.New(errMsgMinValueConstraint)}
	}
	if obj.MemorySwappiness > 100 {
		return &ParsingError{Err: errors.New(errMsgMaxValueConstraint)}
	}
	if obj.ShmSize < 0 {
		return &ParsingError{Err: errors.New(errMsgMinValueConstraint)}
