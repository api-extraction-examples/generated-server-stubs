...
type TaskSpecPlacement struct {
	...
	Preferences []TaskSpecPlacementPreferencesInner `json:"Preferences,omitempty"`
}

// AssertTaskSpecPlacementRequired checks if the required fields are not zero-ed
func AssertTaskSpecPlacementRequired(obj TaskSpecPlacement) error {
	for _, el := range obj.Platforms {
		if err := AssertPlatformRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Preferences {
		if err := AssertTaskSpecPlacementPreferencesInnerRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertTaskSpecPlacementConstraints checks if the values respects the defined constraints
func AssertTaskSpecPlacementConstraints(obj TaskSpecPlacement) error {
	return nil
}
