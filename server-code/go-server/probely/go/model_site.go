...
type Site struct {
	...
	Cookies []CookiesInner `json:"cookies,omitempty"`

	// Custom description for the resource
	Desc string `json:"desc,omitempty"`

	// Form login field name, field input pairs
	FormLogin []FormLoginInner `json:"form_login,omitempty"`
	...
	Headers []HeadersInner `json:"headers,omitempty"`
...
func AssertSiteRequired(obj Site) error {
	if err := AssertBasicAuthRequired(obj.BasicAuth); err != nil {
		return err
	}
	if err := AssertChangedByRequired(obj.ChangedBy); err != nil {
		return err
	}
	for _, el := range obj.Cookies {
		if err := AssertCookiesInnerRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.FormLogin {
		if err := AssertFormLoginInnerRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Headers {
		if err := AssertHeadersInnerRequired(el); err != nil {
			return err
		}
	}
	return nil
