...
// TargetsAPIController binds http requests to an api service and writes the service results to the http response
type TargetsAPIController struct {
	service TargetsAPIServicer
	errorHandler ErrorHandler
}

// TargetsAPIOption for how the controller is set up.
type TargetsAPIOption func(*TargetsAPIController)

// WithTargetsAPIErrorHandler inject ErrorHandler into controller
func WithTargetsAPIErrorHandler(h ErrorHandler) TargetsAPIOption {
	return func(c *TargetsAPIController) {
		c.errorHandler = h
	}
}

// NewTargetsAPIController creates a default api controller
func NewTargetsAPIController(s TargetsAPIServicer, opts ...TargetsAPIOption) Router {
	controller := &TargetsAPIController{
		service:      s,
...
// Routes returns all the api routes for the TargetsAPIController
...
func (c *TargetsAPIController) TargetsGet(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	var pageParam int32
	if query.Has("page") {
	...
	result, err := c.service.TargetsGet(r.Context(), pageParam, lengthParam, orderingParam, searchParam)
...
func (c *TargetsAPIController) TargetsIdDelete(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	idParam := params["id"]
	if idParam == "" {
	...
	result, err := c.service.TargetsIdDelete(r.Context(), idParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *TargetsAPIController) TargetsIdGet(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	idParam := params["id"]
	if idParam == "" {
		c.errorHandler(w, r, &RequiredError{"id"}, nil)
	...
	result, err := c.service.TargetsIdGet(r.Context(), idParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *TargetsAPIController) TargetsIdPatch(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	idParam := params["id"]
	if idParam == "" {
		c.errorHandler(w, r, &RequiredError{"id"}, nil)
	...
	if err := d.Decode(&targetParam); err != nil && !errors.Is(err, io.EOF) {
		c.errorHandler(w, r, &ParsingError{Err: err}, nil)
		return
	}
	if err := AssertTargetRequired(targetParam); err != nil {
		c.errorHandler(w, r, err, nil)
		return
	}
	if err := AssertTargetConstraints(targetParam); err != nil {
		c.errorHandler(w, r, err, nil)
		return
	}
	result, err := c.service.TargetsIdPatch(r.Context(), idParam, targetParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *TargetsAPIController) TargetsIdPut(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	idParam := params["id"]
	if idParam == "" {
		c.errorHandler(w, r, &RequiredError{"id"}, nil)
	...
	result, err := c.service.TargetsIdPut(r.Context(), idParam, targetParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *TargetsAPIController) TargetsPost(w http.ResponseWriter, r *http.Request) {
	targetParam := Target{}
	d := json.NewDecoder(r.Body)
	d.DisallowUnknownFields()
	if err := d.Decode(&targetParam); err != nil && !errors.Is(err, io.EOF) {
	...
	result, err := c.service.TargetsPost(r.Context(), targetParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
