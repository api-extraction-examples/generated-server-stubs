...
// ArchiveAPIController binds http requests to an api service and writes the service results to the http response
type ArchiveAPIController struct {
	service ArchiveAPIServicer
	errorHandler ErrorHandler
}

// ArchiveAPIOption for how the controller is set up.
type ArchiveAPIOption func(*ArchiveAPIController)

// WithArchiveAPIErrorHandler inject ErrorHandler into controller
func WithArchiveAPIErrorHandler(h ErrorHandler) ArchiveAPIOption {
	return func(c *ArchiveAPIController) {
		c.errorHandler = h
	}
}

// NewArchiveAPIController creates a default api controller
func NewArchiveAPIController(s ArchiveAPIServicer, opts ...ArchiveAPIOption) Router {
	controller := &ArchiveAPIController{
		service:      s,
...
// Routes returns all the api routes for the ArchiveAPIController
func (c *ArchiveAPIController) Routes() Routes {
	return Routes{
		"TargetsActivatePost": Route{
			strings.ToUpper("Post"),
			"/targets/activate/",
			c.TargetsActivatePost,
		},
		"TargetsArchivePost": Route{
			strings.ToUpper("Post"),
			"/targets/archive/",
			c.TargetsArchivePost,
		},
		"TargetsArchivedPost": Route{
			strings.ToUpper("Post"),
			"/targets/archived/",
			c.TargetsArchivedPost,
		},
	}
}

// TargetsActivatePost - Activate targets
func (c *ArchiveAPIController) TargetsActivatePost(w http.ResponseWriter, r *http.Request) {
	...
	result, err := c.service.TargetsActivatePost(r.Context(), requestBodyParam)
...
// TargetsArchivePost - Archive targets
func (c *ArchiveAPIController) TargetsArchivePost(w http.ResponseWriter, r *http.Request) {
	requestBodyParam := []string{}
	d := json.NewDecoder(r.Body)
	d.DisallowUnknownFields()
	if err := d.Decode(&requestBodyParam); err != nil {
	...
	result, err := c.service.TargetsArchivePost(r.Context(), requestBodyParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
// TargetsArchivedPost - List archived targets
func (c *ArchiveAPIController) TargetsArchivedPost(w http.ResponseWriter, r *http.Request) {
	requestBodyParam := []string{}
	d := json.NewDecoder(r.Body)
	d.DisallowUnknownFields()
	if err := d.Decode(&requestBodyParam); err != nil {
	...
	result, err := c.service.TargetsArchivedPost(r.Context(), requestBodyParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
