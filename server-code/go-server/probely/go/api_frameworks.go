...
// FrameworksAPIController binds http requests to an api service and writes the service results to the http response
type FrameworksAPIController struct {
	service FrameworksAPIServicer
	errorHandler ErrorHandler
}

// FrameworksAPIOption for how the controller is set up.
type FrameworksAPIOption func(*FrameworksAPIController)

// WithFrameworksAPIErrorHandler inject ErrorHandler into controller
func WithFrameworksAPIErrorHandler(h ErrorHandler) FrameworksAPIOption {
	return func(c *FrameworksAPIController) {
		c.errorHandler = h
	}
}

// NewFrameworksAPIController creates a default api controller
func NewFrameworksAPIController(s FrameworksAPIServicer, opts ...FrameworksAPIOption) Router {
	controller := &FrameworksAPIController{
		service:      s,
...
// Routes returns all the api routes for the FrameworksAPIController
func (c *FrameworksAPIController) Routes() Routes {
	return Routes{
		"FrameworksGet": Route{
			strings.ToUpper("Get"),
			"/frameworks/",
...
// FrameworksGet - List frameworks
func (c *FrameworksAPIController) FrameworksGet(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	var pageParam int32
	if query.Has("page") {
	...
	result, err := c.service.FrameworksGet(r.Context(), pageParam, lengthParam, orderingParam, searchParam)
...
// FrameworksIdGet - Retrieve framework
func (c *FrameworksAPIController) FrameworksIdGet(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	idParam := params["id"]
	if idParam == "" {
	...
	result, err := c.service.FrameworksIdGet(r.Context(), idParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
