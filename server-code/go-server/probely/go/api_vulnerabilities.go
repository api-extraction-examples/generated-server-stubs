...
// VulnerabilitiesAPIController binds http requests to an api service and writes the service results to the http response
type VulnerabilitiesAPIController struct {
	service VulnerabilitiesAPIServicer
	errorHandler ErrorHandler
}

// VulnerabilitiesAPIOption for how the controller is set up.
type VulnerabilitiesAPIOption func(*VulnerabilitiesAPIController)

// WithVulnerabilitiesAPIErrorHandler inject ErrorHandler into controller
func WithVulnerabilitiesAPIErrorHandler(h ErrorHandler) VulnerabilitiesAPIOption {
	return func(c *VulnerabilitiesAPIController) {
		c.errorHandler = h
	}
}

// NewVulnerabilitiesAPIController creates a default api controller
func NewVulnerabilitiesAPIController(s VulnerabilitiesAPIServicer, opts ...VulnerabilitiesAPIOption) Router {
	controller := &VulnerabilitiesAPIController{
		service:      s,
...
// Routes returns all the api routes for the VulnerabilitiesAPIController
func (c *VulnerabilitiesAPIController) Routes() Routes {
	return Routes{
		"VulnerabilityDefinitionsGet": Route{
			strings.ToUpper("Get"),
			"/vulnerability_definitions/",
			c.VulnerabilityDefinitionsGet,
		},
		"VulnerabilityDefinitionsIdGet": Route{
			strings.ToUpper("Get"),
			"/vulnerability_definitions/{id}/",
			c.VulnerabilityDefinitionsIdGet,
		},
	}
}

// VulnerabilityDefinitionsGet - List vulnerability definitions
func (c *VulnerabilitiesAPIController) VulnerabilityDefinitionsGet(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	var pageParam int32
	if query.Has("page") {
	...
	result, err := c.service.VulnerabilityDefinitionsGet(r.Context(), pageParam, lengthParam, orderingParam, searchParam)
...
// VulnerabilityDefinitionsIdGet - Retrieve vulnerability definition
func (c *VulnerabilitiesAPIController) VulnerabilityDefinitionsIdGet(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	idParam := params["id"]
	if idParam == "" {
	...
	result, err := c.service.VulnerabilityDefinitionsIdGet(r.Context(), idParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
