...
func (c *RoleAPIController) DeleteRole(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	roleNameParam := params["role_name"]
	if roleNameParam == "" {
		c.errorHandler(w, r, &RequiredError{"role_name"}, nil)
		return
	}
	result, err := c.service.DeleteRole(r.Context(), roleNameParam)
...
func (c *RoleAPIController) GetRole(w http.ResponseWriter, r *http.Request) {
	...
	result, err := c.service.GetRole(r.Context(), roleNameParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
// GetRoles - List roles
func (c *RoleAPIController) GetRoles(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	...
	result, err := c.service.GetRoles(r.Context(), limitParam, offsetParam, orderByParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *RoleAPIController) PatchRole(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	query := r.URL.Query()
	roleNameParam := params["role_name"]
	if roleNameParam == "" {
	...
	result, err := c.service.PatchRole(r.Context(), roleNameParam, roleParam, updateMaskParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *RoleAPIController) PostRole(w http.ResponseWriter, r *http.Request) {
	roleParam := Role{}
	d := json.NewDecoder(r.Body)
	d.DisallowUnknownFields()
	if err := d.Decode(&roleParam); err != nil {
	...
	result, err := c.service.PostRole(r.Context(), roleParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
