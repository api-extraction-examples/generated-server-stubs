...
type DatasetEvent struct {
	...
	SourceMapIndex *int32 `json:"source_map_index,omitempty"`
...
// AssertDatasetEventRequired checks if the required fields are not zero-ed
func AssertDatasetEventRequired(obj DatasetEvent) error {
	for _, el := range obj.CreatedDagruns {
		if err := AssertBasicDagRunRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertDatasetEventConstraints checks if the values respects the defined constraints
func AssertDatasetEventConstraints(obj DatasetEvent) error {
	return nil
}
