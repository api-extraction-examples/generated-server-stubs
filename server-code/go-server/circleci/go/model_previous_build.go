/*
 * CircleCI REST API
 *
 * The CircleCI API is a RESTful, fully-featured API that allows you to do almost anything in CircleCI. You can access all information and trigger all actions. The only thing we don’t provide access to is billing functions, which must be done from the CircleCI web UI. 
 *
 * API version: v1
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi




// PreviousBuild - previous build
type PreviousBuild struct {

	BuildNum int32 `json:"build_num,omitempty"`

	BuildTimeMillis int32 `json:"build_time_millis,omitempty"`

	Status Status `json:"status,omitempty"`
}

// AssertPreviousBuildRequired checks if the required fields are not zero-ed
func AssertPreviousBuildRequired(obj PreviousBuild) error {
	return nil
}

// AssertPreviousBuildConstraints checks if the values respects the defined constraints
func AssertPreviousBuildConstraints(obj PreviousBuild) error {
	return nil
}
