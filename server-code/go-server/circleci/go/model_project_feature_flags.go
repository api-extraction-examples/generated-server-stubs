/*
 * CircleCI REST API
 *
 * The CircleCI API is a RESTful, fully-featured API that allows you to do almost anything in CircleCI. You can access all information and trigger all actions. The only thing we don’t provide access to is billing functions, which must be done from the CircleCI web UI. 
 *
 * API version: v1
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi




type ProjectFeatureFlags struct {

	BuildForkPrs bool `json:"build-fork-prs,omitempty"`

	Fleet *bool `json:"fleet,omitempty"`

	Junit bool `json:"junit,omitempty"`

	Oss bool `json:"oss,omitempty"`

	Osx bool `json:"osx,omitempty"`

	SetGithubStatus bool `json:"set-github-status,omitempty"`

	TrustyBeta bool `json:"trusty-beta,omitempty"`
}

// AssertProjectFeatureFlagsRequired checks if the required fields are not zero-ed
func AssertProjectFeatureFlagsRequired(obj ProjectFeatureFlags) error {
	return nil
}

// AssertProjectFeatureFlagsConstraints checks if the values respects the defined constraints
func AssertProjectFeatureFlagsConstraints(obj ProjectFeatureFlags) error {
	return nil
}
