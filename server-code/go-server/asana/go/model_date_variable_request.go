/*
 * Asana
 *
 * This is the interface for interacting with the [Asana Platform](https://developers.asana.com). Our API reference is generated from our [OpenAPI spec] (https://raw.githubusercontent.com/Asana/developer-docs/master/defs/asana_oas.yaml).
 *
 * API version: 1.0
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi


import (
	"time"
)



type DateVariableRequest struct {

	// Globally unique identifier of the date field in the project template. A value of `1` refers to the project start date, while `2` refers to the project due date.
	Gid string `json:"gid,omitempty"`

	// The date with which the date variable should be replaced when instantiating a project. This takes a date with `YYYY-MM-DD` format.
	Value *time.Time `json:"value,omitempty"`
}

// AssertDateVariableRequestRequired checks if the required fields are not zero-ed
func AssertDateVariableRequestRequired(obj DateVariableRequest) error {
	return nil
}

// AssertDateVariableRequestConstraints checks if the values respects the defined constraints
func AssertDateVariableRequestConstraints(obj DateVariableRequest) error {
	return nil
}
