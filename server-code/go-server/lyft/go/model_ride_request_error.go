/*
 * Lyft
 *
 * Drive your app to success with Lyft's API
 *
 * API version: 1.0.0
 * Contact: api-support@lyft.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi




// RideRequestError - Details about why a request failed, such as missing or invalid parameters
type RideRequestError struct {

	// A token that confirms the user has accepted current Prime Time and/or fixed price charges
	CostToken string `json:"cost_token,omitempty"`

	// A \"slug\" that serves as the error code (eg. \"bad_parameter\")
	Error string `json:"error"`

	// A user-friendly description of the error (appropriate to show to an end-user)
	ErrorDescription string `json:"error_description,omitempty"`

	ErrorDetail []ErrorDetail `json:"error_detail,omitempty"`

	// When a user must go through another flow before requesting a ride, this URI specifies which flow to use (e.g. an account challenge flow in a web view)
	ErrorUri string `json:"error_uri,omitempty"`

	// A token that confirms the user has accepted current Prime Time charges (Deprecated)
	PrimetimeConfirmationToken string `json:"primetime_confirmation_token,omitempty"`

	// Current Prime Time multiplier (eg. if primetime_percentage is 100%, primetime_multiplier will be 2.0)
	PrimetimeMultiplier float32 `json:"primetime_multiplier,omitempty"`

	// Current Prime Time percentage
	PrimetimePercentage string `json:"primetime_percentage,omitempty"`

	// Validity of the token in seconds
	TokenDuration string `json:"token_duration,omitempty"`
}

// AssertRideRequestErrorRequired checks if the required fields are not zero-ed
func AssertRideRequestErrorRequired(obj RideRequestError) error {
	elements := map[string]interface{}{
		"error": obj.Error,
	}
	for name, el := range elements {
		if isZero := IsZeroValue(el); isZero {
			return &RequiredError{Field: name}
		}
	}

	for _, el := range obj.ErrorDetail {
		if err := AssertErrorDetailRequired(el); err != nil {
			return err
		}
	}
	return nil
}

// AssertRideRequestErrorConstraints checks if the values respects the defined constraints
func AssertRideRequestErrorConstraints(obj RideRequestError) error {
	return nil
}
