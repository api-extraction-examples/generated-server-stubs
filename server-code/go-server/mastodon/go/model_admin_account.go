/*
 * Mastodon API Specification (https://github.com/mastodon/mastodon)
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * API version: 1.0
 * Contact: sardo@hey.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi


import (
	"time"
)



// AdminAccount - Admin-level information about a given account.
type AdminAccount struct {

	Account Account `json:"account,omitempty"`

	// Whether the account is currently approved.
	Approved bool `json:"approved,omitempty"`

	// Whether the account has confirmed their email address.
	Confirmed bool `json:"confirmed,omitempty"`

	// When the account was first discovered.
	CreatedAt time.Time `json:"created_at,omitempty"`

	// The ID of the application that created this account. Cast from an integer, but not guaranteed to be a number.
	CreatedByApplicationId string `json:"created_by_application_id,omitempty"`

	// Whether the account is currently disabled.
	Disabled bool `json:"disabled,omitempty"`

	// The email address associated with the account.
	Email string `json:"email,omitempty"`

	// The ID of the account in the database. Cast from an integer, but not guaranteed to be a number.
	Id string `json:"id,omitempty"`

	// Invite request text ???
	InviteRequest string `json:"invite_request,omitempty"`

	// The ID of the account that invited this user. Cast from an integer, but not guaranteed to be a number.
	InvitedByAccountId string `json:"invited_by_account_id,omitempty"`

	// The IP address last used to login to this account.
	Ip string `json:"ip,omitempty"`

	// The locale of the account. ISO 639 Part 1 two-letter language code.
	Locale string `json:"locale,omitempty"`

	// The current role of the account. Enumerable oneOf.
	Role string `json:"role,omitempty"`

	// Whether the account is currently silenced.
	Silenced bool `json:"silenced,omitempty"`

	// Whether the account is currently suspended.
	Suspended bool `json:"suspended,omitempty"`

	// The username of the account.
	Username string `json:"username,omitempty"`
}

// AssertAdminAccountRequired checks if the required fields are not zero-ed
func AssertAdminAccountRequired(obj AdminAccount) error {
	if err := AssertAccountRequired(obj.Account); err != nil {
		return err
	}
	return nil
}

// AssertAdminAccountConstraints checks if the values respects the defined constraints
func AssertAdminAccountConstraints(obj AdminAccount) error {
	return nil
}
