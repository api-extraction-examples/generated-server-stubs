/*
 * Mastodon API Specification (https://github.com/mastodon/mastodon)
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * API version: 1.0
 * Contact: sardo@hey.com
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapi


import (
	"time"
)



// FeaturedTag - Represents a hashtag that is featured on a profile.
type FeaturedTag struct {

	// The internal ID of the featured tag in the database. Cast from integer but not guaranteed to be a number
	Id string `json:"id,omitempty"`

	// The timestamp of the last authored status containing this hashtag. ISO 8601 Datetime.
	LastStatusAt time.Time `json:"last_status_at,omitempty"`

	// The name of the hashtag being featured.
	Name string `json:"name,omitempty"`

	// The number of authored statuses containing this hashtag.
	StatusesCount int32 `json:"statuses_count,omitempty"`

	// A link to all statuses by a user that contain this hashtag.
	Url string `json:"url,omitempty"`
}

// AssertFeaturedTagRequired checks if the required fields are not zero-ed
func AssertFeaturedTagRequired(obj FeaturedTag) error {
	return nil
}

// AssertFeaturedTagConstraints checks if the values respects the defined constraints
func AssertFeaturedTagConstraints(obj FeaturedTag) error {
	return nil
}
