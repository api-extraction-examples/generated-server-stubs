/*
 * Netlify's API documentation
 *
 * Netlify is a hosting service for the programmable web. It understands your documents and provides an API to handle atomic deploys of websites, manage form submissions, inject JavaScript snippets, and much more. This is a REST-style API that uses JSON for serialization and OAuth 2 for authentication.  This document is an OpenAPI reference for the Netlify API that you can explore. For more detailed instructions for common uses, please visit the [online documentation](https://www.netlify.com/docs/api/). Visit our Community forum to join the conversation about [understanding and using Netlify’s API](https://community.netlify.com/t/common-issue-understanding-and-using-netlifys-api/160).  Additionally, we have two API clients for your convenience: - [Go Client](https://github.com/netlify/open-api#go-client) - [JS Client](https://github.com/netlify/build/tree/main/packages/js-client)
 *
 * The version of the OpenAPI document: 2.16.0
 * 
 * Generated by: https://openapi-generator.tech
 */

using System;
using System.Linq;
using System.Text;
using System.Collections.Generic;
using System.ComponentModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Org.OpenAPITools.Converters;

namespace Org.OpenAPITools.Models
{ 
    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public partial class DeployFiles : IEquatable<DeployFiles>
    {
        /// <summary>
        /// Gets or Sets VarAsync
        /// </summary>
        [DataMember(Name="async", EmitDefaultValue=true)]
        public bool VarAsync { get; set; }

        /// <summary>
        /// Gets or Sets Branch
        /// </summary>
        [DataMember(Name="branch", EmitDefaultValue=false)]
        public string Branch { get; set; }

        /// <summary>
        /// Gets or Sets Draft
        /// </summary>
        [DataMember(Name="draft", EmitDefaultValue=true)]
        public bool Draft { get; set; }

        /// <summary>
        /// Gets or Sets Files
        /// </summary>
        [DataMember(Name="files", EmitDefaultValue=false)]
        public Object Files { get; set; }

        /// <summary>
        /// Gets or Sets Framework
        /// </summary>
        [DataMember(Name="framework", EmitDefaultValue=false)]
        public string Framework { get; set; }

        /// <summary>
        /// Gets or Sets FunctionSchedules
        /// </summary>
        [DataMember(Name="function_schedules", EmitDefaultValue=false)]
        public List<FunctionSchedule> FunctionSchedules { get; set; }

        /// <summary>
        /// Gets or Sets Functions
        /// </summary>
        [DataMember(Name="functions", EmitDefaultValue=false)]
        public Object Functions { get; set; }

        /// <summary>
        /// Gets or Sets FunctionsConfig
        /// </summary>
        [DataMember(Name="functions_config", EmitDefaultValue=false)]
        public Dictionary<string, FunctionConfig> FunctionsConfig { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class DeployFiles {\n");
            sb.Append("  VarAsync: ").Append(VarAsync).Append("\n");
            sb.Append("  Branch: ").Append(Branch).Append("\n");
            sb.Append("  Draft: ").Append(Draft).Append("\n");
            sb.Append("  Files: ").Append(Files).Append("\n");
            sb.Append("  Framework: ").Append(Framework).Append("\n");
            sb.Append("  FunctionSchedules: ").Append(FunctionSchedules).Append("\n");
            sb.Append("  Functions: ").Append(Functions).Append("\n");
            sb.Append("  FunctionsConfig: ").Append(FunctionsConfig).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (obj is null) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((DeployFiles)obj);
        }

        /// <summary>
        /// Returns true if DeployFiles instances are equal
        /// </summary>
        /// <param name="other">Instance of DeployFiles to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(DeployFiles other)
        {
            if (other is null) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    VarAsync == other.VarAsync ||
                    
                    VarAsync.Equals(other.VarAsync)
                ) && 
                (
                    Branch == other.Branch ||
                    Branch != null &&
                    Branch.Equals(other.Branch)
                ) && 
                (
                    Draft == other.Draft ||
                    
                    Draft.Equals(other.Draft)
                ) && 
                (
                    Files == other.Files ||
                    Files != null &&
                    Files.Equals(other.Files)
                ) && 
                (
                    Framework == other.Framework ||
                    Framework != null &&
                    Framework.Equals(other.Framework)
                ) && 
                (
                    FunctionSchedules == other.FunctionSchedules ||
                    FunctionSchedules != null &&
                    other.FunctionSchedules != null &&
                    FunctionSchedules.SequenceEqual(other.FunctionSchedules)
                ) && 
                (
                    Functions == other.Functions ||
                    Functions != null &&
                    Functions.Equals(other.Functions)
                ) && 
                (
                    FunctionsConfig == other.FunctionsConfig ||
                    FunctionsConfig != null &&
                    other.FunctionsConfig != null &&
                    FunctionsConfig.SequenceEqual(other.FunctionsConfig)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    
                    hashCode = hashCode * 59 + VarAsync.GetHashCode();
                    if (Branch != null)
                    hashCode = hashCode * 59 + Branch.GetHashCode();
                    
                    hashCode = hashCode * 59 + Draft.GetHashCode();
                    if (Files != null)
                    hashCode = hashCode * 59 + Files.GetHashCode();
                    if (Framework != null)
                    hashCode = hashCode * 59 + Framework.GetHashCode();
                    if (FunctionSchedules != null)
                    hashCode = hashCode * 59 + FunctionSchedules.GetHashCode();
                    if (Functions != null)
                    hashCode = hashCode * 59 + Functions.GetHashCode();
                    if (FunctionsConfig != null)
                    hashCode = hashCode * 59 + FunctionsConfig.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(DeployFiles left, DeployFiles right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(DeployFiles left, DeployFiles right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
