from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from openapi_server.models.base_model import Model
from openapi_server.models.drive_end_time import DriveEndTime
from openapi_server import util

from openapi_server.models.drive_end_time import DriveEndTime  # noqa: E501

class Drive(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, defense=None, defense_conference=None, drive_number=None, drive_result=None, end_defense_score=None, end_offense_score=None, end_period=None, end_time=None, end_yardline=None, end_yards_to_goal=None, game_id=None, id=None, is_home_offense=None, offense=None, offense_conference=None, plays=None, scoring=None, start_defense_score=None, start_offense_score=None, start_period=None, start_time=None, start_yardline=None, start_yards_to_goal=None, yards=None):  # noqa: E501
        """Drive - a model defined in OpenAPI

        :param defense: The defense of this Drive.  # noqa: E501
        :type defense: str
        :param defense_conference: The defense_conference of this Drive.  # noqa: E501
        :type defense_conference: str
        :param drive_number: The drive_number of this Drive.  # noqa: E501
        :type drive_number: int
        :param drive_result: The drive_result of this Drive.  # noqa: E501
        :type drive_result: str
        :param end_defense_score: The end_defense_score of this Drive.  # noqa: E501
        :type end_defense_score: int
        :param end_offense_score: The end_offense_score of this Drive.  # noqa: E501
        :type end_offense_score: int
        :param end_period: The end_period of this Drive.  # noqa: E501
        :type end_period: int
        :param end_time: The end_time of this Drive.  # noqa: E501
        :type end_time: DriveEndTime
        :param end_yardline: The end_yardline of this Drive.  # noqa: E501
        :type end_yardline: int
        :param end_yards_to_goal: The end_yards_to_goal of this Drive.  # noqa: E501
        :type end_yards_to_goal: int
        :param game_id: The game_id of this Drive.  # noqa: E501
        :type game_id: int
        :param id: The id of this Drive.  # noqa: E501
        :type id: int
        :param is_home_offense: The is_home_offense of this Drive.  # noqa: E501
        :type is_home_offense: bool
        :param offense: The offense of this Drive.  # noqa: E501
        :type offense: str
        :param offense_conference: The offense_conference of this Drive.  # noqa: E501
        :type offense_conference: str
        :param plays: The plays of this Drive.  # noqa: E501
        :type plays: int
        :param scoring: The scoring of this Drive.  # noqa: E501
        :type scoring: bool
        :param start_defense_score: The start_defense_score of this Drive.  # noqa: E501
        :type start_defense_score: int
        :param start_offense_score: The start_offense_score of this Drive.  # noqa: E501
        :type start_offense_score: int
        :param start_period: The start_period of this Drive.  # noqa: E501
        :type start_period: int
        :param start_time: The start_time of this Drive.  # noqa: E501
        :type start_time: DriveEndTime
        :param start_yardline: The start_yardline of this Drive.  # noqa: E501
        :type start_yardline: int
        :param start_yards_to_goal: The start_yards_to_goal of this Drive.  # noqa: E501
        :type start_yards_to_goal: int
        :param yards: The yards of this Drive.  # noqa: E501
        :type yards: int
        """
        self.openapi_types = {
            'defense': str,
            'defense_conference': str,
            'drive_number': int,
            'drive_result': str,
            'end_defense_score': int,
            'end_offense_score': int,
            'end_period': int,
            'end_time': DriveEndTime,
            'end_yardline': int,
            'end_yards_to_goal': int,
            'game_id': int,
            'id': int,
            'is_home_offense': bool,
            'offense': str,
            'offense_conference': str,
            'plays': int,
            'scoring': bool,
            'start_defense_score': int,
            'start_offense_score': int,
            'start_period': int,
            'start_time': DriveEndTime,
            'start_yardline': int,
            'start_yards_to_goal': int,
            'yards': int
        }

        self.attribute_map = {
            'defense': 'defense',
            'defense_conference': 'defense_conference',
            'drive_number': 'drive_number',
            'drive_result': 'drive_result',
            'end_defense_score': 'end_defense_score',
            'end_offense_score': 'end_offense_score',
            'end_period': 'end_period',
            'end_time': 'end_time',
            'end_yardline': 'end_yardline',
            'end_yards_to_goal': 'end_yards_to_goal',
            'game_id': 'game_id',
            'id': 'id',
            'is_home_offense': 'is_home_offense',
            'offense': 'offense',
            'offense_conference': 'offense_conference',
            'plays': 'plays',
            'scoring': 'scoring',
            'start_defense_score': 'start_defense_score',
            'start_offense_score': 'start_offense_score',
            'start_period': 'start_period',
            'start_time': 'start_time',
            'start_yardline': 'start_yardline',
            'start_yards_to_goal': 'start_yards_to_goal',
            'yards': 'yards'
        }

        self._defense = defense
        self._defense_conference = defense_conference
        self._drive_number = drive_number
        self._drive_result = drive_result
        self._end_defense_score = end_defense_score
        self._end_offense_score = end_offense_score
        self._end_period = end_period
        self._end_time = end_time
        self._end_yardline = end_yardline
        self._end_yards_to_goal = end_yards_to_goal
        self._game_id = game_id
        self._id = id
        self._is_home_offense = is_home_offense
        self._offense = offense
        self._offense_conference = offense_conference
        self._plays = plays
        self._scoring = scoring
        self._start_defense_score = start_defense_score
        self._start_offense_score = start_offense_score
        self._start_period = start_period
        self._start_time = start_time
        self._start_yardline = start_yardline
        self._start_yards_to_goal = start_yards_to_goal
        self._yards = yards

    @classmethod
    def from_dict(cls, dikt) -> 'Drive':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The Drive of this Drive.  # noqa: E501
        :rtype: Drive
        """
        return util.deserialize_model(dikt, cls)

    @property
    def defense(self) -> str:
        """Gets the defense of this Drive.


        :return: The defense of this Drive.
        :rtype: str
        """
        return self._defense

    @defense.setter
    def defense(self, defense: str):
        """Sets the defense of this Drive.


        :param defense: The defense of this Drive.
        :type defense: str
        """

        self._defense = defense

    @property
    def defense_conference(self) -> str:
        """Gets the defense_conference of this Drive.


        :return: The defense_conference of this Drive.
        :rtype: str
        """
        return self._defense_conference

    @defense_conference.setter
    def defense_conference(self, defense_conference: str):
        """Sets the defense_conference of this Drive.


        :param defense_conference: The defense_conference of this Drive.
        :type defense_conference: str
        """

        self._defense_conference = defense_conference

    @property
    def drive_number(self) -> int:
        """Gets the drive_number of this Drive.


        :return: The drive_number of this Drive.
        :rtype: int
        """
        return self._drive_number

    @drive_number.setter
    def drive_number(self, drive_number: int):
        """Sets the drive_number of this Drive.


        :param drive_number: The drive_number of this Drive.
        :type drive_number: int
        """

        self._drive_number = drive_number

    @property
    def drive_result(self) -> str:
        """Gets the drive_result of this Drive.


        :return: The drive_result of this Drive.
        :rtype: str
        """
        return self._drive_result

    @drive_result.setter
    def drive_result(self, drive_result: str):
        """Sets the drive_result of this Drive.


        :param drive_result: The drive_result of this Drive.
        :type drive_result: str
        """

        self._drive_result = drive_result

    @property
    def end_defense_score(self) -> int:
        """Gets the end_defense_score of this Drive.


        :return: The end_defense_score of this Drive.
        :rtype: int
        """
        return self._end_defense_score

    @end_defense_score.setter
    def end_defense_score(self, end_defense_score: int):
        """Sets the end_defense_score of this Drive.


        :param end_defense_score: The end_defense_score of this Drive.
        :type end_defense_score: int
        """

        self._end_defense_score = end_defense_score

    @property
    def end_offense_score(self) -> int:
        """Gets the end_offense_score of this Drive.


        :return: The end_offense_score of this Drive.
        :rtype: int
        """
        return self._end_offense_score

    @end_offense_score.setter
    def end_offense_score(self, end_offense_score: int):
        """Sets the end_offense_score of this Drive.


        :param end_offense_score: The end_offense_score of this Drive.
        :type end_offense_score: int
        """

        self._end_offense_score = end_offense_score

    @property
    def end_period(self) -> int:
        """Gets the end_period of this Drive.


        :return: The end_period of this Drive.
        :rtype: int
        """
        return self._end_period

    @end_period.setter
    def end_period(self, end_period: int):
        """Sets the end_period of this Drive.


        :param end_period: The end_period of this Drive.
        :type end_period: int
        """

        self._end_period = end_period

    @property
    def end_time(self) -> DriveEndTime:
        """Gets the end_time of this Drive.


        :return: The end_time of this Drive.
        :rtype: DriveEndTime
        """
        return self._end_time

    @end_time.setter
    def end_time(self, end_time: DriveEndTime):
        """Sets the end_time of this Drive.


        :param end_time: The end_time of this Drive.
        :type end_time: DriveEndTime
        """

        self._end_time = end_time

    @property
    def end_yardline(self) -> int:
        """Gets the end_yardline of this Drive.


        :return: The end_yardline of this Drive.
        :rtype: int
        """
        return self._end_yardline

    @end_yardline.setter
    def end_yardline(self, end_yardline: int):
        """Sets the end_yardline of this Drive.


        :param end_yardline: The end_yardline of this Drive.
        :type end_yardline: int
        """

        self._end_yardline = end_yardline

    @property
    def end_yards_to_goal(self) -> int:
        """Gets the end_yards_to_goal of this Drive.


        :return: The end_yards_to_goal of this Drive.
        :rtype: int
        """
        return self._end_yards_to_goal

    @end_yards_to_goal.setter
    def end_yards_to_goal(self, end_yards_to_goal: int):
        """Sets the end_yards_to_goal of this Drive.


        :param end_yards_to_goal: The end_yards_to_goal of this Drive.
        :type end_yards_to_goal: int
        """

        self._end_yards_to_goal = end_yards_to_goal

    @property
    def game_id(self) -> int:
        """Gets the game_id of this Drive.


        :return: The game_id of this Drive.
        :rtype: int
        """
        return self._game_id

    @game_id.setter
    def game_id(self, game_id: int):
        """Sets the game_id of this Drive.


        :param game_id: The game_id of this Drive.
        :type game_id: int
        """

        self._game_id = game_id

    @property
    def id(self) -> int:
        """Gets the id of this Drive.


        :return: The id of this Drive.
        :rtype: int
        """
        return self._id

    @id.setter
    def id(self, id: int):
        """Sets the id of this Drive.


        :param id: The id of this Drive.
        :type id: int
        """

        self._id = id

    @property
    def is_home_offense(self) -> bool:
        """Gets the is_home_offense of this Drive.


        :return: The is_home_offense of this Drive.
        :rtype: bool
        """
        return self._is_home_offense

    @is_home_offense.setter
    def is_home_offense(self, is_home_offense: bool):
        """Sets the is_home_offense of this Drive.


        :param is_home_offense: The is_home_offense of this Drive.
        :type is_home_offense: bool
        """

        self._is_home_offense = is_home_offense

    @property
    def offense(self) -> str:
        """Gets the offense of this Drive.


        :return: The offense of this Drive.
        :rtype: str
        """
        return self._offense

    @offense.setter
    def offense(self, offense: str):
        """Sets the offense of this Drive.


        :param offense: The offense of this Drive.
        :type offense: str
        """

        self._offense = offense

    @property
    def offense_conference(self) -> str:
        """Gets the offense_conference of this Drive.


        :return: The offense_conference of this Drive.
        :rtype: str
        """
        return self._offense_conference

    @offense_conference.setter
    def offense_conference(self, offense_conference: str):
        """Sets the offense_conference of this Drive.


        :param offense_conference: The offense_conference of this Drive.
        :type offense_conference: str
        """

        self._offense_conference = offense_conference

    @property
    def plays(self) -> int:
        """Gets the plays of this Drive.


        :return: The plays of this Drive.
        :rtype: int
        """
        return self._plays

    @plays.setter
    def plays(self, plays: int):
        """Sets the plays of this Drive.


        :param plays: The plays of this Drive.
        :type plays: int
        """

        self._plays = plays

    @property
    def scoring(self) -> bool:
        """Gets the scoring of this Drive.


        :return: The scoring of this Drive.
        :rtype: bool
        """
        return self._scoring

    @scoring.setter
    def scoring(self, scoring: bool):
        """Sets the scoring of this Drive.


        :param scoring: The scoring of this Drive.
        :type scoring: bool
        """

        self._scoring = scoring

    @property
    def start_defense_score(self) -> int:
        """Gets the start_defense_score of this Drive.


        :return: The start_defense_score of this Drive.
        :rtype: int
        """
        return self._start_defense_score

    @start_defense_score.setter
    def start_defense_score(self, start_defense_score: int):
        """Sets the start_defense_score of this Drive.


        :param start_defense_score: The start_defense_score of this Drive.
        :type start_defense_score: int
        """

        self._start_defense_score = start_defense_score

    @property
    def start_offense_score(self) -> int:
        """Gets the start_offense_score of this Drive.


        :return: The start_offense_score of this Drive.
        :rtype: int
        """
        return self._start_offense_score

    @start_offense_score.setter
    def start_offense_score(self, start_offense_score: int):
        """Sets the start_offense_score of this Drive.


        :param start_offense_score: The start_offense_score of this Drive.
        :type start_offense_score: int
        """

        self._start_offense_score = start_offense_score

    @property
    def start_period(self) -> int:
        """Gets the start_period of this Drive.


        :return: The start_period of this Drive.
        :rtype: int
        """
        return self._start_period

    @start_period.setter
    def start_period(self, start_period: int):
        """Sets the start_period of this Drive.


        :param start_period: The start_period of this Drive.
        :type start_period: int
        """

        self._start_period = start_period

    @property
    def start_time(self) -> DriveEndTime:
        """Gets the start_time of this Drive.


        :return: The start_time of this Drive.
        :rtype: DriveEndTime
        """
        return self._start_time

    @start_time.setter
    def start_time(self, start_time: DriveEndTime):
        """Sets the start_time of this Drive.


        :param start_time: The start_time of this Drive.
        :type start_time: DriveEndTime
        """

        self._start_time = start_time

    @property
    def start_yardline(self) -> int:
        """Gets the start_yardline of this Drive.


        :return: The start_yardline of this Drive.
        :rtype: int
        """
        return self._start_yardline

    @start_yardline.setter
    def start_yardline(self, start_yardline: int):
        """Sets the start_yardline of this Drive.


        :param start_yardline: The start_yardline of this Drive.
        :type start_yardline: int
        """

        self._start_yardline = start_yardline

    @property
    def start_yards_to_goal(self) -> int:
        """Gets the start_yards_to_goal of this Drive.


        :return: The start_yards_to_goal of this Drive.
        :rtype: int
        """
        return self._start_yards_to_goal

    @start_yards_to_goal.setter
    def start_yards_to_goal(self, start_yards_to_goal: int):
        """Sets the start_yards_to_goal of this Drive.


        :param start_yards_to_goal: The start_yards_to_goal of this Drive.
        :type start_yards_to_goal: int
        """

        self._start_yards_to_goal = start_yards_to_goal

    @property
    def yards(self) -> int:
        """Gets the yards of this Drive.


        :return: The yards of this Drive.
        :rtype: int
        """
        return self._yards

    @yards.setter
    def yards(self, yards: int):
        """Sets the yards of this Drive.


        :param yards: The yards of this Drive.
        :type yards: int
        """

        self._yards = yards
